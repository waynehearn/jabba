...
echo "    "
echo "    # Detect if we're on Windows (Git Bash/MSYS/Cygwin) or Unix/Linux/macOS"
echo "    case \"\$OSTYPE\" in"
echo "        msys*|cygwin*|win32)"
echo "            # Windows: use .exe extension and --fd3 flag (fd redirection doesn't work)"
echo "            JABBA_SHELL_INTEGRATION=ON \"$JABBA_HOME_TO_EXPORT/bin/jabba.exe\" \"\$@\" --fd3 \"\$fd3\""
echo "            local exit_code=\$?"
echo "            if [ -s \"\$fd3\" ]; then"
echo "                # Convert Windows paths to Unix-style for Git Bash"
echo "                # 1. Replace backslashes with forward slashes"
echo "                # 2. Convert C:/ to /c/ (drive letters)"
echo "                # 3. Replace semicolons with colons (PATH separator)"
echo "                eval \$(cat \"\$fd3\" | sed 's#\\\\#/#g' | sed 's#\([A-Za-z]\):/#/\L\1/#g' | sed 's#;#:#g')"
echo "            fi"
echo "            ;;"
echo "        *)"
echo "            # Unix/Linux/macOS: use fd redirection (standard approach)"
